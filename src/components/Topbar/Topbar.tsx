...
function Topbar() {
  ...
  const {
    data: dataGlobal,
    isLoading,
  }: { data: MarketData; isLoading: boolean } = useFetchAPI(
    `/global?centralized=true`
  );
      ...
      {!isLoading ? (
        <div className="flex items-center gap-3 text-12 w-full animate-fade">
          <div className="flex items-center gap-1">
            <span className="font-medium text-typo-1 whitespace-nowrap">
              {t(`topbar.cryptos`)}:
            </span>
            <span className="text-primary-1 whitespace-nowrap">
              {formatQuoteCurrency(
                Number(dataGlobal.data.active_cryptocurrencies)
              )}
            </span>
          </div>
          <div className="flex items-center gap-1">
            ...
            <span className="text-primary-1 whitespace-nowrap">
              {formatCurrencyHasUnit(
                Number(dataGlobal.data.total_market_cap["usd"])
              )}
            </span>
          </div>
          <div className="flex items-center gap-1">
            ...
            >
              {Number(
                dataGlobal.data.market_cap_change_percentage_24h_usd
              ).toFixed(2)}
              %
            </span>
          </div>
