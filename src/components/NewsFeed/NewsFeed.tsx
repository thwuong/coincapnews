        ...
        >
          ...
          <div className="flex flex-col gap-3">
            <div className="flex flex-col">
              <h3 className="text-lg font-semibold text-typo-4 hover:text-primary-1">
                {post_title}
              </h3>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-1">
                <h6 className="text-base leading-[26px] font-semibold text-typo-4">
                  {author.name}
                </h6>
                <span>&#183;</span>
                <span className="text-13 leading-[26px] text-typo-2 font-medium">
                  {post_date}
                </span>
              </div>
            </div>
...
function NewsFeed({
  ...
  const [limit, setLimit] = useState(
    Number(process.env.NEXT_PUBLIC_NEWS_PER_PAGE)
  );
  const {
    data,
    isLoading,
  }: { data: FeedType[]; error: any; isLoading: boolean } = useNewsAPI(
    `?limit=${limit}&category=${category}`
  );
  const { t } = useTranslation(currentLanguage);
  return (
    <div className="flex flex-col gap-6 w-full">
      <h1 className="text-2xl font-bold text-typo-4">{t(titleKey)}</h1>
