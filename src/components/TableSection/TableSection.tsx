...
const CoinTable = dynamic(() =>
  import("../CoinTable").then((mod) => mod.CoinTable)
);

const columnHelper = createColumnHelper<CoinType>();
const skipFirstRowSortingFn: SortingFn<CoinType> = (
  rowA: any,
  rowB: any,
  columnId: string
) => {
...
function TableSection() {
  ...
  const [pageCount, setPageCount] = useState<number>(() =>
    Math.ceil(
      Number(process.env.NEXT_PUBLIC_TOTAL) /
        Number(process.env.NEXT_PUBLIC_PER_PAGE)
    )
  );
  ...
  return (
    <section className="flex flex-col items-center gap-8 w-full">
      <section className="flex items-center justify-between w-full">
        ...
        >
          <Image
            ...
            height={14}
          />
          <span className="text-12 font-bold max-lg:hidden">
            {t("watchlist")}
          </span>
        </Box>
      ...
      <CoinTable
        ...
        isLoading={isLoading}
        currentPage={page}
        skipFirstRowSortingFn={skipFirstRowSortingFn}
      />
