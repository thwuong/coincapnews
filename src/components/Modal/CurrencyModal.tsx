...
function CurrencyModal({ isOpen, onClose }: CurrencyModalProps) {
  ...
  const keysFiat = React.useMemo(() => {
    if (!data) return null;
    const objectToArray = Object.keys(data.rates).map((key) => [
      key,
      data.rates[key],
    ]);
    ...
    return fiatList.filter(
      (value) =>
        ...
        String(value[1]?.name.toLocaleLowerCase()).includes(
  ...
  const keysCrypto = React.useMemo(() => {
    ...
    const cryptoList = objectToArray.filter(
      (value) => value[1].type === "crypto"
    );
        ...
        <ModalBody>
          ...
          <div className="grid items-start  grid-cols-2 overflow-y-auto max-h-[420px] scroll-hidden">
            ...
            <div className="grid grid-cols-2 col-span-1 gap-3">
                ...
                keysCrypto.map((item, index) => (
                  ...
                  >
                    <div className="flex flex-col w-[90%] items-start gap-0.5">
                      <p className="text-typo-1 font-medium text-sm uppercase">
                        {item[0]}
                      </p>
              ...
              {keysCrypto?.length === 0 && (
                <p className="text-typo-1 text-12 px-3">Items not found</p>
              )}
            </div>
            {/* Fiat Currencies */}
            <div className="grid grid-cols-2 col-span-1 gap-3">
