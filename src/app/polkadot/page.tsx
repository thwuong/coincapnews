...
export async function generateMetadata({ params }: PageProps, parent: ResolvingMetadata): Promise<Metadata> {
    ...
    const content = {
        meta: {
            title: "Polkadot Eco | Coincapnews",
            description: "",
        },
    };
    ...
    return {
        ...
        openGraph: {
            ...
            title: content.meta?.title || title || "",
            description: content.meta?.description || description || "",
            url: `${WEBSITE_HOST_URL}/polkadot?lang=${lang}`,
            locale: "en-US",
            siteName: content.meta?.title,
            type: "website",
        },
        alternates: {
            canonical: `${WEBSITE_HOST_URL}/polkadot?lang=${lang}`,
        },
        twitter: {
    ...
    return (
        <main className="pb-24">
            <Container className="px-12">
                <Heading className="pt-8 pb-12 max-lg:py-8" title={t("title.polkadot_eco")} />
                <CommonSection category="Polkadot Ecosystem" />
            </Container>
        </main>
    );
}
